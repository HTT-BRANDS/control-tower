...
**Compliance Posture:**
- OWASP ASVS Level 2: ~92% coverage (gap: password hashing library not enforced for internal dev login; SBOM generation closed 2026-04-23 via bd 7mk8)
...
| SBOM generation | ✅ Pass | SPDX-JSON SBOM generated by Syft and Sigstore-attested on every prod build (`.github/workflows/deploy-production.yml`). Closed 2026-04-23 via bd 7mk8. |
...
### M-3: No SBOM Generation (Medium) — ✅ CLOSED 2026-04-23

**Original risk:** No Software Bill of Materials was generated during the
build process. This limited supply chain visibility per NIST SSDF and SOC 2
CC7.1 requirements.

**Resolution (bd 7mk8):** `.github/workflows/deploy-production.yml` now:
1. Generates SPDX-JSON SBOM via `anchore/sbom-action` (Syft) (commit `7d816f6`)
2. Attests the SBOM via Sigstore keyless signing (`actions/attest-sbom`)
3. Also generates SLSA v1 build provenance (`actions/attest-build-provenance`,
   commit `7921b92`)
4. `cosign verify-attestation` runs fail-closed on BOTH attestations in the
   deploy job before any `az webapp config container set` (commit `b28a9f2`)

The 4-claim verification policy (subject digest, predicate type, OIDC cert
identity, OIDC issuer) lives machine-readable at `arbiter/policies/verify.yaml`.

**Remaining follow-ups (tracked):**
- Dependency-track integration for continuous SBOM monitoring (not opened — ship if a real zero-day response gap is observed).
...
- [x] SBOM generation in CI/CD (Finding M-3 — closed 2026-04-23 via bd 7mk8)
...
| M-3 | CI/CD pipeline review | ✅ Syft SBOM + Sigstore attestation on every prod build — verified at deploy time by cosign (closed 2026-04-23, bd 7mk8) |
...
3. ~~**M-3:** Add SBOM generation (`cyclonedx-py`) to CI/CD pipeline~~ — ✅ CLOSED 2026-04-23 (Syft + Sigstore)
...
- ~~**M-3** (No SBOM)~~: ✅ CLOSED 2026-04-23 (bd 7mk8) — Syft SBOM + Sigstore attestation wired and verified at deploy time
...
**Final Verdict: Ship it 🚀** — Low residual risk. Address M-1 before first production deploy. ~~M-3~~ (closed 2026-04-23) and L-findings are tracked in the regular backlog.
