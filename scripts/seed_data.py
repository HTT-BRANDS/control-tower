...
RESOURCE_TYPES = [
    ("Microsoft.Compute/virtualMachines", "vm", ["Standard_D2s_v3", "Standard_B2s", "Standard_D4s_v3"]),
...
SERVICE_NAMES = [
    ...
    "Azure Functions", "Key Vault", "Virtual Network", "Container Registry",
...
ADMIN_NAMES = [
    "Tyler Granlund", "Sarah Chen", "Marcus Williams", "Jessica Patel",
    "David Kim", "Emily Rodriguez", "James Thompson", "Aisha Mohammed",
    "Robert Garcia", "Lisa Nakamura", "Chris Anderson", "Priya Sharma",
]
ADMIN_ROLES = [
    "Global Administrator", "User Administrator", "Security Administrator",
    "Billing Administrator", "Application Administrator",
...
def seed_compliance(db, tenants: list[dict]):
    ...
    for t in tenants:
        ...
        for day_offset in range(30):
            ...
            trend = day_offset * 0.1  # Gets better as days increase (going back in time, so reverse)
        ...
        for policy_name, category, violation_rate in POLICY_DEFS:
            ...
            db.add(PolicyState(
                ...
                recommendation=f"Review {violations} resources for {policy_name.lower()}" if violations else None,
                synced_at=past(0),
            ))
    ...
    for t in tenants:
        ...
        for i in range(resource_count):
            ...
            if random.random() > 0.3:
                tags["CostCenter"] = random.choice(["CC-HTT-001", "CC-BCC-002", "CC-FN-003", "CC-TLL-004"])
            if random.random() > 0.4:
                tags["Application"] = random.choice(["governance-platform", "salon-booking", "e-commerce", "crm"])
            ...
            db.add(Resource(
                ...
                estimated_monthly_cost=round(random.uniform(5, 800), 2) if random.random() > 0.3 else None,
                synced_at=past(0),
            ))
        ...
        for rid in random.sample(resource_ids, max(2, resource_count // 8)):
            db.add(IdleResource(
                ...
                tenant_id=t["id"],
                subscription_id=sub["id"],
                idle_type=random.choice(["low_cpu", "no_connections", "zero_traffic", "unused_disk"]),
                description=random.choice([
...
def seed_sync_history(db, tenants: list[dict]):
    ...
    for jt in job_types:
        ...
        durations = []
        for day_offset in range(30):
            for run in range(random.randint(1, 2)):
    ...
    # Alerts
    alert_templates = [
        ("sync_failure", "warning", "Compliance sync failed for {tenant}", "Sync returned HTTP 429. Retry scheduled in 5 minutes."),
        ("stale_sync", "warning", "Cost data stale for {tenant}", "Last successful sync was 8 hours ago — threshold is 6 hours."),
        ("high_error_rate", "error", "High error rate on identity sync", "12% error rate over the last 24 hours (threshold: 5%)."),
        ("no_records", "info", "No new resources detected for {tenant}", "Resources sync completed but found 0 new records — verify API permissions."),
    ]
...
def seed_recommendations(db, tenants: list[dict]):
    ...
    rec_templates = [
        ("cost_optimization", "idle_vm", "Shut down idle VM {name}", "VM has < 2% CPU utilization over 14 days", "High", 180),
        ("cost_optimization", "right_size_vm", "Right-size {name} from D4s to D2s", "VM consistently uses < 25% of allocated CPU", "Medium", 95),
        ("cost_optimization", "reserved_instance", "Purchase RI for {name}", "VM has been running 24/7 for 90+ days", "High", 420),
        ("security", "unencrypted_storage", "Enable encryption on {name}", "Storage account does not have encryption at rest enabled", "Critical", None),
        ("security", "open_nsg", "Restrict NSG rules on {name}", "Network security group allows inbound traffic on all ports", "Critical", None),
        ("performance", "slow_query", "Optimize SQL queries on {name}", "P95 query latency > 2s on 3 queries", "Medium", None),
        ("reliability", "no_backup", "Enable backup for {name}", "SQL database has no point-in-time restore configured", "High", None),
    ]
    ...
    for t in tenants:
        for _ in range(random.randint(3, 8)):
            ...
            db.add(Recommendation(
                ...
                description=tmpl[3],
                impact=tmpl[4],
                potential_savings_monthly=round(tmpl[5] * random.uniform(0.8, 1.3), 2) if tmpl[5] else None,
                potential_savings_annual=round(tmpl[5] * 12 * random.uniform(0.8, 1.3), 2) if tmpl[5] else None,
                resource_name=name,
    ...
    for t in tenants:
        ...
        if random.random() > 0.5:
            db.add(DMARCAlert(
                ...
                alert_type=random.choice(["policy_none", "dkim_disabled", "high_failure_rate", "spf_misconfigured"]),
...
def seed_riverside(db, tenants: list[dict]):
    ...
    for t in tenants:
        ...
        db.add(RiversideDeviceCompliance(
            tenant_id=t["id"],
