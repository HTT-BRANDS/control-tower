...
class TestCostServiceAnomalies:
    ...
    def test_get_anomalies_acknowledged_only(
        self, cost_service, mock_db, sample_anomalies
    ):
    ...
    def test_get_anomalies_unacknowledged_only(
        self, cost_service, mock_db, sample_anomalies
    ):
    ...
    def test_get_top_anomalies_empty(
        self, cost_service, mock_db, sample_tenants
    ):
    ...
    @pytest.mark.asyncio
    async def test_acknowledge_anomaly_success(
        self, cost_service, mock_db, sample_anomalies
    ):
        ...
        with patch("app.core.cache.invalidate_on_sync_completion", new_callable=AsyncMock):
            # Execute
            result = await cost_service.acknowledge_anomaly(
                anomaly_id=1, user="admin@test.com"
            )
    ...
    @pytest.mark.asyncio
    async def test_acknowledge_anomaly_not_found(
        ...
        # Execute
        result = await cost_service.acknowledge_anomaly(
            anomaly_id=999, user="admin@test.com"
        )
    ...
    @pytest.mark.asyncio
    async def test_bulk_acknowledge_partial_failure(
        self, cost_service, mock_db
    ):
        ...
        with patch.object(
            cost_service, "acknowledge_anomaly", side_effect=mock_acknowledge
        ):
            # Execute
            result = await cost_service.bulk_acknowledge_anomalies(
                anomaly_ids=anomaly_ids, user="admin@test.com"
