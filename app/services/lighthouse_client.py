...
class LighthouseAzureClient:
        ...
        try:
            ...
            if subscription:
                ...
                return {
                    ...
                    "state": None,
                    "tenant_id": None,
                    "error": "Subscription not accessible via Lighthouse delegation",
                }
    ...
    def _get_subscription_sync(self, subscription_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.debug(f"Subscription {subscription_id} not accessible: {e}")
            return None
        ...
        try:
            if resource_group:
                ...
                for resource in client.resources.list_by_resource_group(resource_group):
                    resources.append(self._resource_to_dict(resource))
            else:
    ...
    async def get_cost_data(
        ...
        Returns:
            Dictionary with cost data:
            {
                ...
                "start_date": str,
                "end_date": str,
                "rows": list[dict]  # Detailed cost breakdown
            }
        ...
        try:
            ...
            logger.info(f"Retrieved cost data for {subscription_id}: {cost_data['total_cost']} {cost_data['currency']}")
            return cost_data
        ...
        try:
            ...
            if result.rows:
                for row in result.rows:
                    # Azure cost query returns: [Cost, Currency, Date, ResourceGroup, ServiceName]
                    row_dict = {
                        "cost": float(row[0]) if len(row) > 0 else 0.0,
                        "currency": str(row[1]) if len(row) > 1 else "USD",
                        "date": str(row[2]) if len(row) > 2 else None,
                        "resource_group": str(row[3]) if len(row) > 3 else None,
                        "service_name": str(row[4]) if len(row) > 4 else None,
                    }
